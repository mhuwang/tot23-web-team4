 ...
 * @LastEditors: zqy
 * @LastEditTime: 2021-04-15 22:46:53
-->
    ...
    <el-card class="box-card">
      ...
      <List item-layout="horizontal" :split="false">
        <div class="metadata-item">
          <p>名字</p>
          <span>{{ replicaSet.metadata.name }}</span>
        </div>
        <div class="metadata-item">
          <p>命名空间</p>
          <span>{{ replicaSet.metadata.namespace }}</span>
        </div>
        <div class="metadata-item">
          <p>创建时间</p>
          <span>{{
            replicaSet.metadata.creationTimestamp.replaceAll(/[TZ]/g, " ")
          }}</span>
        </div>
        <div class="metadata-item">
          <p>UID</p>
          <span>{{ replicaSet.metadata.uid }}</span>
        </div>
      </List>
      <!-- 元数据 标签 注释部分 -->
      <List item-layout="horizontal" :split="false">
...
export default {
  ...
  mounted: function () {
    ...
    this.$store
      ...
      .then((res) => {
        console.log(res);
        this.replicaSet = res.dataReplicaSet;
        this.pods = res.dataPods;
        this.services = res.dataServices;
        // this.replicaSetStatus = res.data.status.conditions;
        this.loading = false;
