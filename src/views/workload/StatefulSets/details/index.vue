 ...
 * @LastEditors: zqy
 * @LastEditTime: 2021-04-15 23:04:20
-->
  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <List item-layout="horizontal" :split="false">
        <div class="metadata-item">
          <p>名字</p>
          <span>{{ statefulSet.metadata.name }}</span>
        </div>
        <div class="metadata-item">
          <p>命名空间</p>
          <span>{{ statefulSet.metadata.namespace }}</span>
        </div>
        <div class="metadata-item">
          <p>创建时间</p>
          <span>{{
            statefulSet.metadata.creationTimestamp.replaceAll(/[TZ]/g, " ")
          }}</span>
        </div>
        <div class="metadata-item">
          <p>UID</p>
          <span>{{ statefulSet.metadata.uid }}</span>
        </div>
      </List>
      <!-- 元数据 标签 注释部分 -->
      <List item-layout="horizontal" :split="false">
    ...
    <el-card class="box-card">
      ...
      <List item-layout="horizontal" :split="false">
        ...
        <div class="metadata-item" v-if="statefulSet.spec.replicas">
          <p>期望</p>
          <span>{{ statefulSet.spec.replicas }}</span>
        </div>
      </List>
    </el-card>
    <br /><br />
...
export default {
  ...
  mounted: function () {
    ...
    let nameAndNamespace = {
      // name: sessionStorage.getItem("statefulSetName"),
      // namespace: sessionStorage.getItem("statefulSetNamespace"),
      name: this.name.split(",")[0],
      namespace: this.name.split(",")[1],
    };
    this.$store
      .dispatch("statefulSets/getStatefulSetResources", nameAndNamespace)
      .then((res) => {
        console.log(res);
        this.statefulSet = res.dataStatefulSet;
        this.pods = res.dataPods;
        this.services = res.dataServices;
        // this.statefulSetStatus = res.data.status.conditions;
        this.loading = false;
        // console.log(this.statefulSetStatus)
      })
      .catch((error) => {
        throw error;
      });
  ...
  computed: {
    ...
    labels() {
      let labelArr = [];
      for (let pro in this.statefulSet.metadata.labels) {
        labelArr.push({
          key: pro,
          value: this.statefulSet.metadata.labels[pro],
        });
      }
      return labelArr;
    },
    ...
    annotations() {
      let annoArr = [];
      for (let anno in this.statefulSet.metadata.annotations) {
        annoArr.push({
          key: anno,
          value: this.statefulSet.metadata.annotations[anno],
        });
      }
      return annoArr;
    },
    ...
    matchLabels() {
      let matchLabelsList = [];
      for (let i in this.statefulSet.spec.selector.matchLabels) {
        matchLabelsList.push({
          key: i,
          value: this.statefulSet.spec.selector.matchLabels[i],
        });
      }
    ...
    images() {
      ...
      for (
        let i = 0;
        i < this.statefulSet.spec.template.spec.containers.length;
        i++
      ) {
        imagesList.push(this.statefulSet.spec.template.spec.containers[i].image);
      }
      return imagesList;
    },
  ...
  methods: {
    ...
    showAnnoDetails: function (key) {
      ...
      this.annoDetails = this.beautify(
        this.statefulSet.metadata.annotations[key],
        {
          indent_size: 2,
