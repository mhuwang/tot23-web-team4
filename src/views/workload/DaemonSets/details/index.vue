  ...
  <dev>
    ...
    <el-card class="box-card">
      ...
      <List item-layout="horizontal" :split="false">
        ...
        <div class="metadata-item" v-if="daemonSet.status.numberMisscheduled > 0">
          <p>错误调度</p>
...
export default {
  ...
  computed: {
    ...
    labels() {
      let labelArr = [];
      for (let pro in this.daemonSet.metadata.labels) {
        labelArr.push({
          key: pro,
    ...
    annotations() {
      let annoArr = [];
      for (let anno in this.daemonSet.metadata.annotations) {
        annoArr.push({
          key: anno,
