 ...
 * @LastEditTime: 2021-03-30 16:52:46
-->
<template>
  <div>
      ...
      <el-table :data="nodes" style="width: 100%" stripe>
        <el-table-column width="40">
          <template slot-scope="scope">
            <svg-icon :icon-class="scope.row.node.status.conditions[3].status == 'True'? 'load-success': scope.row.node.status.conditions[3].status == 'Unknown'?'load-doubt':'load-failed'"/>
          </template>
        </el-table-column>
        <el-table-column label="名字" width="120">
          <template slot-scope="scope">
            <router-link :to="'/summary/nodes/'+scope.row.node.metadata.name" @click.native="goToNodeDetails(scope.row.node.metadata.name)" class="link-type">
              <span style="color:#409EFF;text-decoration:underline">{{ scope.row.node.metadata.name }}</span>
            </router-link>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="node.status.conditions[3].status" width="100" label="准备就绪"> </el-table-column>
        <el-table-column prop="usage.cpu" label="CPU 利用率" width="200">
          <template slot-scope="scope">
            <el-progress class="my-progress" color="#fc8621" :text-inside="true" :stroke-width="20" :percentage="(scope.row.usage.cpu / 1000 / 1000 / scope.row.node.status.allocatable.cpu.amount / 10).toFixed(2)*1.0"></el-progress>
          </template>
        </el-table-column>
        <el-table-column prop="usage.memory" label="内存利用率" width="200">
          <template slot-scope="scope">
            <el-progress class="my-progress" color="#c24914" :text-inside="true" :stroke-width="20" :percentage="(scope.row.usage.memory / scope.row.node.status.allocatable.memory.amount * 100).toFixed(2)*1.0"></el-progress>
          </template>
        </el-table-column>
        
        <el-table-column prop="node.metadata.creationTimestamp" label="创建时间">
          <template slot-scope="scope">
            <span>{{scope.row.node.metadata.creationTimestamp.replaceAll(/[TZ]/g,' ')}}</span>
          </template>
        </el-table-column>
        <el-table-column label="是否可调度" width="100">
            ...
            <template slot-scope="scope">
                <!-- {{scope.row}} 每一行封存的数据 -->
                <el-switch v-model="!scope.row.node.spec.unschedulable" @change="userStateChange(scope.row.node)"></el-switch>
            </template>
        </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
              <!-- 修改 -->
              <el-button style="margin-bottom:5px" type="primary" icon="el-icon-edit" size="small" @click="showNodeEditDialog(scope.row.node)">编辑</el-button>
              <br>
              <!-- 删除 -->
              <el-button type="danger" icon="el-icon-delete" size="small" @click="delNode(scope.row.node)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="clusterRoles" style="width: 100%" stripe>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
              <!-- 修改 -->
              <el-button type="primary" icon="el-icon-edit" size="small" @click="showClusterRolesEditDialog(scope.row)">编辑</el-button>
              <!-- 删除 -->
              <el-button type="danger" icon="el-icon-delete" size="small" @click="delClusterRoles(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    getAllNodes: function () {
      this.$store
        ...
        .then((res) => {
          console.log(res);
          // 过滤掉 edge 节点
           this.nodes = res.data.filter( item => item.node.metadata.name.indexOf('edge') < 0 );
        })
        .catch((error) => {
          console.log(error);
        });
